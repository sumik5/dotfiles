...
### 必須スキル（実装時に必ず参照）
- **`solid-clean-code`**: SOLID原則、クリーンコード、単一責任原則等
- **`type-safety`**: any/Any型禁止、型ガード、厳格な型チェック
- **`testing`**: TDD、AAA パターン、テストカバレッジ100%目標
- **`security-codeguard`**: セキュアコーディング、CodeGuard実行（実装完了後必須）
...
3. `solid-clean-code`、`type-safety`、`security-codeguard` - コード品質向上
...
### 1. 型安全性（必須）
- **any/Any型禁止**: TypeScript/Python共通（`type-safety`スキル参照）
...
- **セキュアコーディング**: `security-codeguard`スキル参照（実装完了後にCodeGuard必須実行）
...
**最終更新**: 2025年1月（Next.js 16.0.0 / React 19.2.0対応）
