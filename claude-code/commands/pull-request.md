...
<!-- コミットメッセージやブランチ名から推測して記載 -->
<!-- Issue番号が分かる場合は resolves #<Issue番号> を記載 -->

## What（変更内容の概要）
何を変更したのかを簡潔に記載

<!-- 変更されたファイルや機能の概要をリスト形式で記載 -->

## How（変更の詳細）
どのように変更したのかを詳細に記載

<!-- 技術的な詳細、実装方法、設計の決定事項などを記載 -->
<!-- 複数のコミットがある場合は、主要な変更点をまとめる -->

## Test（テスト方法）
この変更をテストする方法を記載

<!-- テストケース、確認方法、動作確認手順などを記載 -->
<!-- 自動テストを追加した場合はその旨を記載 -->

## Checklist（確認事項）
- [ ] セルフコードレビューを実施した
- [ ] コードに適切なコメントを追加した（特に複雑な部分）
- [ ] 関連するドキュメントを更新した
- [ ] 変更によって新しい警告が発生しないことを確認した
- [ ] テストを追加した（バグ修正や新機能の場合）
...
- **Why（背景・目的）**: レビュアーが変更の文脈を理解するため
- **What（概要）**: 変更内容を素早く把握するため
- **How（詳細）**: 実装の詳細を理解し、レビューの質を高めるため
- **Test（テスト方法）**: 変更の妥当性を検証する手順を明確にするため
- **Checklist（確認事項）**: レビュー前の品質チェックを標準化するため
