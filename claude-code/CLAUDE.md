...
- **絶対条件**: すべての会話、説明、コード内コメント、エラーメッセージ、報告は**必ず日本語**で記述
- **禁止**: 英語での応答は一切禁止（技術用語・固有名詞を除く）
- **適用範囲**:
  - Claude Code本体のすべての応答
  - すべてのAgent（PO、Manager、Developer）の応答
  - タスク報告、進捗報告、完了報告
  - エラーメッセージ、警告メッセージ
  - コード内のコメント（日本語または英語を選択可能）
- **例外**:
  - プログラミング言語のキーワード（if、function、class等）
  - ライブラリ・フレームワーク名（Next.js、React、FastAPI等）
  - 技術用語の原語表記（必要に応じて日本語説明を併記）

**この言語設定は他のすべての指示より優先されます。**
...
- **設定値**: `"autoCompact": false` （自動コンパクションをオフ）
- **理由**: 予期しないタイミングでのコンテキスト喪失を防ぐため

### トークン使用率の監視（必須）

**CRITICAL: Claude Codeは常にトークン使用率を監視し、適切なタイミングでコンパクションを実行してください。**

#### 監視ルール
1. **定期チェック**: 各応答の前にトークン使用率を確認
   - トークン使用率は画面右上または`<budget:token_budget>`タグで確認可能
   - 現在の使用率 = (使用トークン数 / 総トークン数) × 100

2. **閾値判断**:
   - **95%以上**: 即座にコンパクション実行（緊急）
   - **90%以上**: 次の大きなタスク前にコンパクション推奨
   - **85%以上**: コンパクションを検討

#### コンパクション実行手順（自動化）

**トークン使用率が95%以上に達した場合、以下を自動的に実行：**

```
1. ユーザーに通知:
   「トークン使用率が95%を超えました（現在：XX%）。
   コンパクションを実行し、CLAUDE.mdを再読み込みします。」

2. コンパクション実行:
   - 画面上の「Compact」ボタンをクリック
   - またはコンパクションコマンドを実行

3. /reloadコマンド実行（必須）:
   - コンパクション完了直後に必ず `/reload` を実行
   - これによりCLAUDE.mdの重要な指示を復元

4. 確認メッセージ:
   「コンパクション完了。CLAUDE.mdを再読み込みしました。」
```

#### 重要な注意事項
- ⚠️ **コンパクション後は必ず `/reload` を実行**（自動化必須）
- ⚠️ **95%を超える前に実行推奨**（余裕を持った管理）
- ⚠️ **長時間の会話では定期的にトークン使用率を確認**
- ✅ **コンパクション実行後は必ず動作確認**（CLAUDE.mdの指示が有効か確認）

#### トークン節約のベストプラクティス
1. **serena MCPを最大活用**: ファイル全体読み込みを避ける
2. **不要なファイル読み込みを避ける**: 必要な部分のみを読む
3. **Task toolの活用**: 大規模調査は専用エージェントに委任
4. **定期的なコンパクション**: 90%到達前に実行を検討
