...
| 🔴 対象リポジトリと参照実装リポジトリを横断する作業時（移植・パターン適用・差分移行） | **同名ファイル（`next.config.ts` 等）を並列Readする際は、対象リポジトリ側のフルパスを読み漏らしていないかツール呼び出し直後に確認する**。参照実装側だけを読んで対象の現状と誤認する事故が起きる（実例: 参照側の設定を対象の現状と取り違え、ビルド成果物が生成されない矛盾に直面するまで気づかなかった）。計画書の記述は鵜呑みにせず**対象リポジトリの実ファイルで裏取り**し、実測が期待と食い違った時点で即座に設定ファイルの中身を読み直す |
