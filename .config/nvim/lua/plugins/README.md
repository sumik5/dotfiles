...
**Yank History Picker 内キーマップ**:

| キー | モード | 操作 |
|------|--------|------|
| `<C-j>` / `<C-n>` | Insert / Normal | リスト下移動 |
| `<C-k>` / `<C-p>` | Insert / Normal | リスト上移動 |
| `<C-d>` | Normal（list） | プレビュー下スクロール |
| `<C-u>` | Normal（list） | プレビュー上スクロール |
...
#### 🔍 fzf-lua（Markdownファイル内のみ）

| キー | モード | 操作 |
|------|--------|------|
| `@` | Insert | fzf-luaでファイルを検索し、パスを挿入 |
